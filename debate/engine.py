...
class DebateEngine:
    ...
    def run_segment(self, state: DebateState, stop_event: threading.Event | None = None,
                    on_event: Callable[..., None] | None = None,
                    user_note_provider: Callable[[], str] | None = None) -> dict[str, object]:
        rounds = []
        for round_number in range(1, self.rounds_per_segment + 1):
            ...
            # Human notes are consumed only when the Moderator's turn begins.
            user_note = user_note_provider() if user_note_provider else ""
            mission_result = self.moderator.mission(state, user_note)
            mission = str(mission_result["text"]).strip()
            self._emit(on_event, "moderator_mission", round=round_number, text=mission,
                       finish_reason=mission_result.get("finish_reason"), user_note_consumed=bool(user_note))
            ...
            for name, client in self.agents:
                ...
                self._emit(on_event, "agent_response", round=round_number, agent=name, text=reply,
                           finish_reason=result.get("finish_reason"), truncated=result.get("finish_reason") == "length")

            self._check_stop(stop_event)
            update = self.moderator.update_state(state, mission, round_data["agents"][0]["text"], round_data["agents"][1]["text"])
            round_data["moderator_update"] = update
            self._emit(on_event, "moderator_update", round=round_number, text=str(update["text"]),
                       finish_reason=update.get("finish_reason"), state=self._state_snapshot(state))
            rounds.append(round_data)

        self._check_stop(stop_event)
        summary = self.moderator.summarize_segment(state)
        self._emit(on_event, "moderator_summary", segment=state.segment_number, text=str(summary["text"]),
    ...
    def _check_stop(stop_event: threading.Event | None) -> None:
        if stop_event is not None and stop_event.is_set():
            raise DebateStopped("Debate stopped by user.")
    ...
    def _state_snapshot(state: DebateState) -> dict[str, object]:
        return {"segment": state.segment_number, "round": state.round_number, "consensus": state.consensus,
                "disagreements": state.disagreements, "open_questions": state.open_questions,
                "decisions": state.decisions, "proposals": state.proposals, "risks": state.risks,
